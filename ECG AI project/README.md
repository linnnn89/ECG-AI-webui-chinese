...
**核心模型：** InceptionTime（PyTorch 训练，ONNX 推理）  
**标签方案：**
...
## 十三、后续备选设计：本地病例检索与 LLM 报告层（目前未加入）
...
结果默认只返回 `score >= 0.55` 的病例；不足 10 个时不硬凑，并在 JSON `warnings` 中说明。MVP-1 再考虑 `forward_features()`、ONNX embedding 输出和 embedding 主导的检索。
...
1. 当前 ECG 经 ONNX 得到概率与标签。
2. 程序基于 embedding/logits/prob 向量检索 top-k 相似病例。
...
## 十七、CaseBank 向量索引与 WebUI 检索公开边界
